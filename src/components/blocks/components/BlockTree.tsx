...
import { FaAngleRight, FaArrowDown, FaTimesCircle } from 'react-icons/fa';
...
const BlockTree = (props: any) => {
    ...
    return (editor.current != null && current.id != null) ? (
        <div className={`flex flex-col items-stretch  cursor-pointer  ${current.tag != 'document' ? 'pl-2' : ''} text-xs text-white bg-y-blue-500`} onClick={(e) => {
            ...
            <div className={`flex p-2 border-l border-b border-gray-200 ${editor.current.id === current.id ? ' border bg-primary-600 text-white' : 'text-black'}`}>
                <div className="flex flex-row justify-between items-center w-full truncate">
                    ...
                    <div className="flex flex-row gap-1">
                        {editor.current.tag != 'document' ? <FaTimesCircle onClick={deleteBlock} className={`${editor.current.id === current.id ? 'text-white' : 'text-gray-400'}`} /> : null}
                    </div>
                </div>
            </div>
