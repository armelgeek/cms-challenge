...
const AccentColor = ({ title, data, attr, updateCss }: any) => {
    ...
    <div className="flex flex-row">
      ...
      <span className="uppercase font-bold" style={{
        ...
        onClick={()=> toogleOver(true)}
         className={`  cursor-pointer ${state.color.over.replace('hover:accent', 'accent').replace('hover:', '')} mb-1 w-8 h-8 border-2 rounded-full`}></div>
      </div>
      {state.palette && (
